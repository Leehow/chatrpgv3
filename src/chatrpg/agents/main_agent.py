        ...
        messages = [
            PiMessage(
                ...
                content=(
                    ...
                    "当 context.pending_decisions 中存在 kind=luck_spend 时，先根据玩家整段发言与上下文的语义判断其对该待处理决策的立场。"
                    "如果语义是在消耗幸运修正该失败检定，选择 procedure_id=coc7e.luck_spend；"
                    "如果语义是在不消耗幸运并结束该待处理决策，选择 procedure_id=coc7e.luck_decline。"
                    "不要依赖固定词面、固定短语或固定语言判断；要根据玩家意图与 pending decision 的上下文关系做语义判断。"
                    "可以只传 source_event_id 或 decision_id，必要时也可以不传 roll/target，runtime 会使用 pending decision。"
                    "如果无法判断玩家是否要处理该 pending decision，设置 needs_clarification=true。"
