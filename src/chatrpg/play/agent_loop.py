...
class AgentLoopEngine:
    ...
    async def run(self, request: AgentLoopRunRequest) -> AgentLoopRunResult:
        ...
        loop_history: list[dict[str, object]] = []
        final_intent = IntentFrame(intent="未解析玩家行动", confidence=0.0)
        final_procedure_result: ProcedureExecutionResult | None = None
        final_clue_decision: ClueAcquisitionDecision | None = None
        ...
        for step_index in range(1, request.max_steps + 1):
            ...
            final_intent = self._intent_with_bound_skill_call(intent)
            self._append_trace(
                trace,
                stage=f"AgentLoop[{step_index}].decide",
                summary="GM Agent 选择下一步：调用 Runtime 工具或结束工具循环。",
                data={
                    "intent": final_intent.intent,
                    "procedure_id": final_intent.procedure_id,
                    "skill_calls": [call.model_dump(mode="json") for call in final_intent.skill_calls],
                    "needs_clarification": final_intent.needs_clarification,
                },
            )

            if final_intent.needs_clarification:
                stop_reason = "needs_clarification"
                break
            if final_intent.procedure_id is None:
                final_clue_decision, clue_events, frontier = await self._resolve_clue_tool(
                    request=request,
                    state=state,
                    frontier=frontier,
                    procedure_result=final_procedure_result,
                )
                if clue_events:
                    ...
                    self._append_trace(
                        ...
                        data={"event_types": [event.event_type for event in clue_events]},
                    )
                stop_reason = "no_more_tools"
                break

            tool_key = self._tool_key(final_intent)
            if tool_key in executed_tool_keys:
                self._append_trace(
                    trace,
                    stage=f"AgentLoop[{step_index}].blocked",
                    summary="阻止重复工具调用，避免 ReAct 循环在同一 procedure/input 上打转。",
                    data={"procedure_id": final_intent.procedure_id, "inputs": final_intent.inputs},
                )
            ...
            final_procedure_result = procedure_result
            ...
            if request.adventure is not None and procedure_result.status == "completed":
                ...
                if pending_clue_events:
                    ...
                    self._append_trace(
                        trace,
                        stage=f"AgentLoop[{step_index}].tool.pending_clue",
                        summary="规则工具成功后兑现先前暂存的线索机会。",
            ...
            if clue_events:
                ...
                self._append_trace(
                    trace,
                    stage=f"AgentLoop[{step_index}].tool.adventure_clue",
                    summary="工具结算后根据原始玩家行动进行线索语义匹配与可见性提交。",
                    data={"event_types": [event.event_type for event in clue_events]},
                )
        ...
        return AgentLoopRunResult(
            ...
            clue_decision=final_clue_decision,
            procedure_result=final_procedure_result,
            agent_trace=trace,
