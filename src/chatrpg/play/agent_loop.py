...
class AgentLoopEngine:
    ...
    async def run(self, request: AgentLoopRunRequest) -> AgentLoopRunResult:
        ...
        for step_index in range(1, request.max_steps + 1):
            ...
            raw_intent = await self._agent.resolve_intent(
                PlayerInput(session_id=request.session_id, actor_id=request.actor_id, message=request.player_message, context=context),
            ...
            if intent.procedure_id is None:
                ...
                plan = await MechanicTriggerJudge(self._semantic_matcher).judge(action=action, scene=scene, trace_id=request.trace_id)
                self._append_trace(
                    trace,
    ...
    def _run_mechanic_steps(self, *, request: AgentLoopRunRequest, state: SessionState, steps: list[Any], trace: list[AgentTraceStep], step_index: int) -> ProcedureExecutionResult | None:
        last_result: ProcedureExecutionResult | None = None
        for mechanic_step in steps:
            step_intent = IntentFrame(intent=mechanic_step.reason or mechanic_step.procedure_id, procedure_id=mechanic_step.procedure_id, actor_id=mechanic_step.actor_id, inputs=mechanic_step.inputs, confidence=1.0)
            last_result = self._run_native_procedure(session_id=request.session_id, state=state, intent=step_intent, actor_id=mechanic_step.actor_id or request.actor_id, trace_id=request.trace_id)
            if last_result is None:
                continue
            self._append_trace(trace, stage=f"AgentLoop[{step_index}].tool.mechanic_procedure", summary="执行 MechanicPlan 中的 Runtime procedure。", data={"procedure_id": last_result.procedure_id, "status": last_result.status, "event_types": [event.event_type for event in last_result.events]})
    ...
    @staticmethod
    def _post_process_procedure_events(*, session_id: str, procedure_result: ProcedureExecutionResult, steps: list[Any], trace_id: str) -> list[DomainEvent]:
        ...
        for event in procedure_result.events:
            ...
            if isinstance(applied_damage, int) and applied_damage:
                events.append(DomainEvent(session_id=session_id, event_type="CharacterResourceChanged", actor_id=target_actor_id, payload={"resource_id": "hp", "delta": -applied_damage, "source_event_id": event.id}, trace_id=trace_id))
        return events
